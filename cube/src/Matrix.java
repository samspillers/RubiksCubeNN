...
public class Matrix<E> implements Iterable<E> {
	/*
	 * Matrix elements are store very similarly to an ArrayList Matrix elements are
	 * held in one flattened array. See diagram below for how matrix elements are
	 * stored in array
	 * 
	 * Formatting of matrix: Array: 0 1 2 3 4 ... Matrix: 0 4 8 12 1 5 9 13 2 6 10
	 * 14 3 7 11 15
	 * 
	 * Col and row numbers start at 0 Indexes are given (row #, col #)
	 * 
	 * // Command to convert from matrix indeces to array index Matrix[rowDex,
	 * colDex] = Array[rowDex + colDex * rows]
	 */

	// I don't recall why these are protected, I might've copied this part from
	// ArrayList
	protected int rows;
	protected int cols;
	...
	// Makes a matrix of the given size with data in the array format described
	// above. Throws exception if array doesn't match given matrix size.
	...
	// This returns a rotation matrix for 3d space. Multiplying vectors to these
	// rotational matrices will rotate
	// the vectors around the given axis for the given angle.
	//
	// These really shouldn't return Matrix<Integer> but instead double or long or
	// something, but since I won't
	// be using angles other than 90 or -90, it shouldn't matter. TODO: Fix to be a
	// functional matrix later.
	// Returns a rotational matrix to multiply vectors on, needs axis ('x', 'y', or
	// 'z') and angle in degrees
	public static Matrix<Integer> rotationalMatrix(char axis, float angle) {
		...
		if (axis == 'x') {
			double[] array = { 1, 0, 0, 0, Math.cos(Math.toRadians(angle)), Math.sin(Math.toRadians(angle)), 0,
			...
			double[] array = { Math.cos(Math.toRadians(angle)), 0, Math.sin(Math.toRadians(angle)), 0, 1, 0,
			...
			double[] array = { Math.cos(Math.toRadians(angle)), -Math.sin(Math.toRadians(angle)), 0,
					Math.sin(Math.toRadians(angle)), Math.cos(Math.toRadians(angle)), 0, 0, 0, 1 };
	...
	public void rotateEntriesClock() {
		...
		for (int i = 0; i < rows / 2; i++) { // i for each circle of the matrix
			...
			boolean cont;
			Queue<E> queueLoop = new LinkedList<E>();
			for (int j = 0; j < 2; j++) {
	...
	// Only works on Numbers (excluding byte and short). Returns that number back in
	// an array. Cast the
	// returned array to whichever type you passed in. idk how to make this return a
	// E[]
	public Number[] multVec(E[] vector) {
	...
	public void setRow(int index, E[] row) {
		if (index < 0 || index >= cols || row.length != cols) {
			throw new IllegalArgumentException();
		}
		for (int i = index; i < cols * rows; i += rows) {
			data[i] = row[(i - index) / rows];
		}
	}

	public E[] getRow(int index, Class<E> theClass) {
		if (index < 0 || index >= cols) {
			throw new IllegalArgumentException();
		}
		E[] output = (E[]) Array.newInstance(theClass, cols);
		for (int i = index; i < cols * rows; i += rows) {
			output[(i - index) / rows] = (E) data[i];
		}
		return output;
	}
	
	public void setCol(int index, E[] col) {
		if (index < 0 || index >= cols || col.length != rows) {
			throw new IllegalArgumentException();
		}
		for (int i = index * cols; i < (index + 1) * cols; i += 1) {
			data[i] = col[i % cols];
		}
	}

	public E[] getCol(int index, Class<E> theClass) {
		if (index < 0 || index >= cols) {
			throw new IllegalArgumentException();
		}
		E[] output = (E[]) Array.newInstance(theClass, rows);
		for (int i = index * cols; i < (index + 1) * cols; i += 1) {
			output[i % cols] = (E) data[i];
		}
		return output;
	}
	...
	private class cIt<E> implements Iterator<E> {
		private int i;
		private Matrix<E> obj;

		private cIt(Matrix<E> obj) {
			i = 0;
			this.obj = obj;
		}
