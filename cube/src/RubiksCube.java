...
public class RubiksCube {
   ...
   */
   ...
   Matrix<Piece[]> cube;
   List<MoveID> movesList;
   private final static int[] upOr = {1, 0, 0};
   private final Matrix<Piece[]> solvedCube;
   private int cubeDim;
   ...
   public RubiksCube(int cubeDim, List<MoveID> moves) {
      ...
      solvedCube = new Matrix<Piece[]>(cube);
   ...
   public boolean pieceCorrectPos(int x, int y, int z) {
      return cube.get(x, y)[z] == solvedCube.get(x, y)[z];
   }
   ...
   private Matrix<Piece[]> newCube(int cubeDim) {
